...
def line_search(x_old, f_old, g, p, function, data, max_step, tol_x=1e-8,
                alpha=1e-4): 

    check = False  

    # Scale if attempted step is too big
    if adipy.linalg.norm(p) > max_step: 
    ...
    while True:
        ...
        alam2 = alam; 
        f2 = f; 
        alam = np.max([tmplam, 0.5*alam])                # try again

def newton(x, function, data, tol_f=1.0e-8, tol_x=1.0e-10, max_iters=1000,
           max_step=100.0):
    ...
    for i in xrange(max_iters):  
        ...
        # solve linear system
        p = adipy.linalg.solve(jac, -fv)

        # perform line search
        x, fv, f, check = line_search(x_old, f_old, g, p, function, data,
                                      stpmax) 
